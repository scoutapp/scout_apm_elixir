defmodule ScoutApm.Tracing.Annotations do
  ...
  defp instrument_transactions(mod) do
    ...
    |> Enum.reverse()
    |> Enum.map(fn %ScoutApm.Tracing.Annotations.Transaction{} = transaction_data ->
      ...
      new_attrs =
        [{:transaction, transaction_data.function_name, transaction_data.args}] ++
          Enum.reverse(attrs)
      ...
      if length(transaction_data.guards) > 0 do
        quote do
          def unquote(transaction_data.function_name)(unquote_splicing(transaction_data.args))
              when unquote_splicing(transaction_data.guards) do
            unquote(body)
          end
        end
      else
  ...
  defp instrument_timings(mod) do
    ...
    |> Enum.reverse()
    |> Enum.map(fn %ScoutApm.Tracing.Annotations.Timing{} = timing_data ->
      ...
      if length(timing_data.guards) > 0 do
        quote do
          def unquote(timing_data.function_name)(unquote_splicing(timing_data.args))
              when unquote_splicing(timing_data.guards) do
            unquote(body)
          end
        end
      else
