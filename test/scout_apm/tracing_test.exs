defmodule ScoutApm.TracingTest do
  ...
  describe "deftiming" do
    test "creates histograms with sensible defaults" do
      ...
      %{reporting_periods: [pid]} = ScoutApm.Store.get()
      Agent.get(pid, fn(%{histograms: histograms}) ->
        assert Map.has_key?(histograms, "Custom/TracingAnnotationTestModule.add_one(integer) when is_integer(integer)")
        assert Map.has_key?(histograms, "Custom/TracingAnnotationTestModule.add_one(number) when is_float(number)")
      end)
    end
